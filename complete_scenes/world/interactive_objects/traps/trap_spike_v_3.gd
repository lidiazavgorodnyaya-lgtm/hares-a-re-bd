...
## Третий вариант напольной ловушки
##
## Активируется, при наступании
## Перезаряжается по истечению таймера
...
func _on_animated_sprite_2d_animation_finished() -> void:
	...
	elif animated_sprite_2d.animation == "spike_of":
		if on_the_sight:
			timer.start(0.2)
	else:
		trap_state = IDLE
...
func damage_inflicted(_damage: float) -> void:
	for body: BaseEntity in target_list:
		#FIXME если это BaseEntity, то сигнал точно есть
